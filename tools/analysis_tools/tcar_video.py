...
def get_predicted_data(sample_data_token: str,
    ...
    box_list = []
    for box in boxes:
        #  Move box to sensor coord system from ego.
...
def _boxes_to_sensor(boxes: List[EvalBox], pose_record: Dict, cs_record: Dict):
    """
    Map boxes from ego coordinate system to lidar
    """
    boxes_out = []
    for box in boxes:
        # Create Box instance.
        ...
        # Move box from ego vehicle to sensor coord system.
        box.rotate(Quaternion(cs_record['rotation']))
        box.translate(np.array(cs_record['translation']))
...
def _visualize_sample(nusc: NuScenes,
                     sample_token: str,
                     gt_boxes: EvalBoxes,
    ...
    boxes_gt = _boxes_to_sensor(boxes_gt_global, pose_record, cs_record)

    # Map EST boxes to lidar.
    boxes_est = _boxes_to_sensor(boxes_est_global, pose_record, cs_record)
    ...
    pc = LidarPointCloud.from_file(osp.join(nusc.dataroot, sd_record['filename']))
...
def _boxes_to_gt_sensor(nusc, boxes: List[Box], sample_data_token: str, box_vis_level):
    """
    Map boxes from ego coordinate system to sensor
    """
    sd_record = nusc.get('sample_data', sample_data_token)
    cs_record = nusc.get('calibrated_sensor', sd_record['calibrated_sensor_token'])
    sensor_record = nusc.get('sensor', cs_record['sensor_token'])
    boxes_out = []
    imsize = (1920, 1080) # width, height
    cam_intrinsic = np.array(cs_record['camera_intrinsic'])
    for box in boxes:
        # Move box from ego vehicle to sensor coord system.
        box.rotate(Quaternion(cs_record['rotation']))
...
def lidar_render(sample_token, data,out_path=None):
    ...
    for ann in anns:
        content = nusc.get('sample_annotation', ann) # ego coordinate
        try:
    ...
    bbox_anns = data['results'][sample_token] # ego coordinate
    ...
    _visualize_sample(nusc, sample_token, gt_annotations, pred_annotations, savepath=out_path+'_bev')
...
def render_sample_data(
    ...
    """
    lidar_render(sample_token, pred_data, out_path=out_path)
    ...
    for ind, cam in enumerate(cams):
        ...
        if sensor_modality in ['lidar', 'radar']:
            ...
            _, boxes_gt, _ = nusc.get_sample_data(sample_data_token, box_vis_level=box_vis_level) # ego coordinate
            boxes_gt = _boxes_to_gt_sensor(nusc, boxes_gt, sample_data_token, box_vis_level=box_vis_level)
...
import cv2
def make_video_from_frames(frame_dir, out_video, fps=12):
    """Collect all *_camera.png frames and encode them into an MP4 video."""
    # Collect frames (sorted by time order)
    frame_paths = sorted([
        osp.join(frame_dir, f) for f in os.listdir(frame_dir)
        if f.endswith('_camera.png')
    ])
    if len(frame_paths) == 0:
        raise RuntimeError("No *_camera.png files found!")
    ...
    vw.release()
    print(f"[INFO] Saved video: {out_video} ({len(frame_paths)} frames @ {fps} FPS)")


if __name__ == '__main__':
    """
    Render all samples in a scene by scene index (not name).
    Example: scene_idx = 5 → the 5th scene in nusc.scene list.
    """
    ...
    bevformer_results = mmcv.load(
        'test/bevformer_tiny_tcar/Thu_Oct__9_12_39_43_2025/pts_bbox/results_nusc.json'
    )
    ...
    scene_idx = 5  # <-- change this to any integer index (0 ~ len(nusc.scene)-1)
    scene_name = val_scenes[scene_idx]

    # Find the matching scene record by name
    scene_record = None
    for s in nusc.scene:
        if s['name'] == scene_name:
            scene_record = s
            break
    scene_record = nusc.scene[val_scenes[scene_idx]]
    scene_token = scene_record['token']
    scene_name = scene_record['name']

    # Collect all sample tokens for that scene
    ...
    print(f"[INFO] Scene index: {scene_idx}  ({scene_name})")
    print(f"[INFO] Number of samples: {len(sample_tokens)}")

    # Create output directory
    out_dir = osp.join('vis_out', f'scene_{scene_idx:03d}')
    ...
    for token in sample_tokens:
        render_sample_data(
            ...
            out_path=osp.join(out_dir, token)
        )
        print(f"[OK] Rendered {token}")
